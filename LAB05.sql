...
-- Câu 1.2: View phân tích hi?u su?t gi?ng d?y v?i RANK/DENSE_RANK và t? l? ??t[cite: 2]
...
-- Câu 1.3: View th?ng kê ??ng ký theo tháng v?i t?ng tích l?y SUM() OVER[cite: 2]
...
-- Câu 1.4: View INSTEAD OF - Cho phép INSERT vào view 4 b?ng[cite: 2]
--B??c 1 - T?o view:
...
--B??c 2 - INSTEAD OF INSERT trigger:
...
-- Câu 1.5: View phân tích phân v? v?i PERCENTILE_CONT và STDDEV[cite: 2]
...
-- Câu 1.6: View PIVOT - ?i?m sinh viên chuy?n thành c?t theo l?p[cite: 2]
...
-- Câu 1.7: View ki?m tra toàn v?n d? li?u b?ng UNION ALL[cite: 2]
...
-- BÀI 2 - STORED PROCEDURE NÂNG CAO[cite: 2]
-- Câu 2.1: Th? t?c tr? v? SYS_REFCURSOR + th? t?c in k?t qu?[cite: 2]
--Th? t?c tr? v? cursor:
...
DBMS_OUTPUT.PUT_LINE('=== KET QUA LOP ' || p_classid || ' ===');
 DBMS_OUTPUT.PUT_LINE(RPAD('Hang',5)||RPAD('Ho  
Ten',22)||LPAD('Diem',6)||' Xep loai');
...
-- Câu 2.2: Exception tùy ch?nh v?i PRAGMA EXCEPTION_INIT[cite: 2]
...
IS
 ...
 ex_class_not_found EXCEPTION; PRAGMA  
EXCEPTION_INIT(ex_class_not_found,-20102);
 ex_class_full EXCEPTION; PRAGMA EXCEPTION_INIT(ex_class_full,  -20103);
 ex_already_enrolled EXCEPTION; PRAGMA  
EXCEPTION_INIT(ex_already_enrolled,-20104);
...
-- Câu 2.3: Th? t?c ?? quy tính h?c phí c?ng d?n tiên quy?t[cite: 2]
...
-- Câu 2.4: Package pkg_student_mgmt - SPEC và BODY ??y ??[cite: 2]
--Package SPEC:
...
END IF;
INSERT INTO
enrollment(studentid,classid,enrolldate,createdby,createddate,modifiedby,mo
difieddate)
VALUES(p_sid,p_cid,SYSDATE,USER,SYSDATE,USER,SYSDATE);
COMMIT;
DBMS_OUTPUT.PUT_LINE('[OK] Dang ky: SV '||p_sid||' -> Lop
'||p_cid);
END enroll;
...
IF v_check=0 THEN DBMS_OUTPUT.PUT_LINE('[LOI] SV chua dang ky lop
nay'); RETURN; END IF;
DELETE FROM enrollment WHERE studentid=p_sid AND classid=p_cid;
COMMIT;
DBMS_OUTPUT.PUT_LINE('[OK] Da huy dang ky: SV '||p_sid||' khoi Lop
'||p_cid);
END withdraw;
...
||' : '||NVL(TO_CHAR(rec.finalgrade),'Chua
co diem'));
END LOOP;
DBMS_OUTPUT.PUT_LINE(' GPA:
'||NVL(TO_CHAR(get_student_gpa(p_sid)),'N/A'));
END;
END pkg_student_mgmt;
/
-- Kiem tra package:
...
-- Câu 2.5: BULK COLLECT và FORALL x? lý hàng lo?t[cite: 2]
...
-- Câu 2.6: Th? t?c báo cáo môn h?c v?i ??nh d?ng b?ng ASCII[cite: 2]
...
BEGIN
 ...
 IF v_check=0 THEN
 DBMS_OUTPUT.PUT_LINE('Mon hoc '||p_courseno||' khong ton tai!');  RETURN;
 END IF;
...
'||TO_CHAR(NVL(v_cost,0),'999,990.00')||' VND');
 ...
 DBMS_OUTPUT.PUT_LINE(v_sep2);
 FOR rec IN (
 SELECT cl.classid, cl.capacity, i.firstname||' '||i.lastname AS ten_gv,
...
-- Câu 2.7: Hàm convert_to_gpa_40 và th? t?c print_gpa_report[cite: 2]
...
-- Câu 2.8: Th? t?c ghi log v?i PRAGMA AUTONOMOUS_TRANSACTION[cite: 2]
...
-- BÀI 3 - TRIGGER NÂNG  CAO[cite: 2]
-- Câu 3.1: Compound Trigger gi?i quy?t Mutating Table[cite: 2]
...
-- Câu 3.2: INSTEAD OF UPDATE Trigger trên view 4 b?ng[cite: 2]
...
-- Câu 3.3: DDL Trigger ghi nh?t ký m?i thay ??i c?u trúc[cite: 2]
...
-- Câu 3.4: Trigger chu?i - Cascade khi xóa sinh viên[cite: 2]
...
-- Câu 3.5: Trigger v?i WHEN - T? ??ng c?p ch?ng ch? khi ??t ?i?m[cite: 2]
...
-- BÀI 4 - T?NG H?P[cite: 2]
-- Câu 4.1: H? th?ng ??ng ký h?c hoàn ch?nh - Package + View + Trigger[cite: 2]
...
--Package pkg_enrollment_system (rút g?n ph?n chính):
...
-- Câu 4.2: T?i ?u hóa: EXPLAIN PLAN và BULK COLLECT Refactor[cite: 2]
--Ph?n A - Phân tích hi?u n?ng v?i EXPLAIN PLAN:
...
--Ph?n B - Refactor cursor b?ng BULK COLLECT:
